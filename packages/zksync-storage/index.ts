...
import { CommitBatchInfo, StoredBatchInfo } from "./types";
...
function hashStoredBatchInfo(batchInfo: StoredBatchInfo): string {
    return keccak256(
...
async function getProofData(
    ...
    if (batchNumber === undefined) {
        batchNumber = await l2Provider.getL1BatchNumber();
    }
    ...
    if (batchMetadata === null) {
        throw new Error(`Batch #${batchNumber} is not in L1`);
    }
...
async function main() {
    ...
    const proof = await getProofData(
        l1Provider,
        l2Provider,
